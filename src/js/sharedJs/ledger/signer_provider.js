...
SignerProvider.prototype.sendAsync = function (payload, callback) {
  ...
  if (payload.method === 'eth_accounts' && self.options.accounts) {
    ...
    self.rpc.sendAsync(
      { method: 'eth_getTransactionCount', params: [payload.params[0].from, 'latest'] },
      (nonceError, nonce) => {
        // eslint-disable-line
        if (nonceError) {
        ...
        self.rpc.sendAsync({ method: 'eth_gasPrice' }, (gasPriceError, gasPrice) => {
          // eslint-disable-line
          if (gasPriceError) {
          ...
          self.options.signTransaction(rawTxPayload, (keyError, signedHexPayload) => {
            // eslint-disable-line
            if (!keyError) {
              // create new output payload
              ...
              callback(
                new Error(
                  `[ethjs-provider-signer] while signing your transaction payload: ${JSON.stringify(keyError)}`
                ),
                null
              );
            }
    ...
    self.options.signPersonalMessage(payload.params[0], (signatureError, signatureResult) => {
      // create new output payload
      const outputPayload = Object.assign(
        {},
        {
      ...
      callback(signatureError, outputPayload);
    });
  } else {
